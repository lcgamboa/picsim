/* ########################################################################
   ...
   Copyright (c) : 2008-2025  Luis Claudio Gamboa Lopes
                    ...
                    switch (opc & 0x00FF) {
                        ...
                        case 0x0005:
                            ...
                            if (((*pic->P18map.STKPTR) & 0x1F) < 31) {
                                ...
                                (*pic->P18map.STKPTR) =
                                    ((*pic->P18map.STKPTR) & 0xC0) | (((*pic->P18map.STKPTR) & 0x1F) + 1);
                                if (((*pic->P18map.STKPTR) & 0x1F) == 31)
                        ...
                        case 0x0008:
                            ...
                            if (pic->print)
                                printf("TBLRD *\n");
                            tblptr =
                                ((*pic->P18map.TBLPTRU) << 16) | ((*pic->P18map.TBLPTRH) << 8) | (*pic->P18map.TBLPTRL);
                        ...
                        case 0x000C:
                            ...
                            if ((tblptr >> 1) < pic->ROMSIZE) {
                                if (tblptr & 0x0001)
                                    pic->prog[tblptr >> 1] = (pic->prog[tblptr >> 1] & 0x00FF) | (*pic->P18map.TABLAT)
                                                                                                     << 8;
                                else
                            ...
                            // write id
                                ...
                                if (tblptr & 0x0001)
                                    pic->id[(tblptr - 0x200000) >> 1] =
                                        (pic->id[(tblptr - 0x200000) >> 1] & 0x00FF) | (*pic->P18map.TABLAT) << 8;
                                else
                                    pic->id[(tblptr - 0x200000) >> 1] =
                                        (pic->id[(tblptr - 0x200000) >> 1] & 0xFF00) | (*pic->P18map.TABLAT);
                            }
                            // write config
                            else if ((tblptr >= 0x300000) && (tblptr <= 0x30000D)) {
                                if (tblptr & 0x0001)
                                    pic->config[(tblptr - 0x300000) >> 1] =
                                        (pic->config[(tblptr - 0x300000) >> 1] & 0x00FF) | (*pic->P18map.TABLAT) << 8;
                                else
                                    pic->config[(tblptr - 0x300000) >> 1] =
                                        (pic->config[(tblptr - 0x300000) >> 1] & 0xFF00) | (*pic->P18map.TABLAT);
                            ...
                            // debug vector
                                ...
                                if (tblptr & 0x0001)
                                    pic->debugv[(tblptr - 0x200028) >> 1] =
                                        (pic->debugv[(tblptr - 0x200028) >> 1] & 0x00FF) | (*pic->P18map.TABLAT) << 8;
                                else
                                    pic->debugv[(tblptr - 0x200028) >> 1] =
                                        (pic->debugv[(tblptr - 0x200028) >> 1] & 0xFF00) | (*pic->P18map.TABLAT);
                            }
        ...
        case 0x2000:
            switch (opc & 0x0C00) {
                case 0x0000:
                    ...
                    fraddr(pic);
                    if ((0xF0 & ((0x0F & ((*pic->P18map.WREG) + pic->ram[raddr])) + ((*pic->P18map.STATUS) & 0x01))) !=
                        0)
                        *status |= 0x02;  // DC=1
                    else
                        *status &= ~0x02;  // DC=0
        ...
        case 0x3000:
            switch (opc & 0x0C00) {
                ...
                case 0x0800:
                    ...
                    pic->rram = raddr;
                    if (opc & 0x0200) {
                        pic->ram[raddr] = ((pic->ram[raddr] & 0x0F) << 4) | ((pic->ram[raddr] & 0xF0) >> 4);
                        pic->lram = raddr;
                    } else {
                        (*pic->P18map.WREG) = ((pic->ram[raddr] & 0x0F) << 4) | ((pic->ram[raddr] & 0xF0) >> 4);
                        pic->lram = sfr_addr(pic->P18map.WREG);
                    }
                    break;
                case 0x0C00:
        ...
        case 0x5000:
            switch (opc & 0x0C00) {
                ...
                case 0x0400:
                    ...
                    fraddr(pic);
                    if ((0xF0 & ((0x0F & (~pic->ram[raddr])) + (!((*pic->P18map.STATUS) & 0x01)) + 1 +
                                 (0x0F & (*pic->P18map.WREG)))) != 0)
                        *status |= 0x02;
                    else
                        *status &= ~0x02;
                ...
                case 0x0800:
                    ...
                    fraddr(pic);
                    if ((0xF0 & ((0x0F & (~(*pic->P18map.WREG))) + (!((*pic->P18map.STATUS) & 0x01)) + 1 +
        ...
        case 0xE000:
            switch (opc & 0x0F00) {
                ...
                case 0x0F00:
                    ...
                    {
                        if (pic->print)
                            printf("GOTO %#012X\n", (((pic->debugv[pic->pc - 0x200027] & 0x0FFF) << 8) | (opc & 0x00FF))
                                                        << 1);
 ...
 if (pic->lram != 0x8000)
  {
        ...
        if (plram == pic->P18map.PREINC0) {
            pic->ram[VALID_ADDR(afsr0 + 1)] = (*pic->P18map.PREINC0);
            if (pic->lram == pic->rram)
                pic->rram = 0x8000;
            pic->lram = VALID_ADDR(afsr0 + 1);
            (*pic->P18map.FSR0H) = ((afsr0 + 1) & 0xFF00) >> 8;
            (*pic->P18map.FSR0L) = (afsr0 + 1) & 0x00FF;
        }
        if (plram == pic->P18map.PLUSW0) {
            pic->lram = VALID_ADDR(afsr0 + ((char)(*pic->P18map.WREG)));
            pic->ram[pic->lram] = (*pic->P18map.PLUSW0);
        }
        ...
        if (plram == pic->P18map.PREINC1) {
            pic->lram = VALID_ADDR(afsr1 + 1);
            pic->ram[pic->lram] = (*pic->P18map.PREINC1);

            (*pic->P18map.FSR1H) = ((afsr1 + 1) & 0xFF00) >> 8;
            (*pic->P18map.FSR1L) = (afsr1 + 1) & 0x00FF;
        }
        if (plram == pic->P18map.PLUSW1) {
            pic->lram = VALID_ADDR(afsr1 + ((char)(*pic->P18map.WREG)));
            pic->ram[pic->lram] = (*pic->P18map.PLUSW1);
        }
        ...
        if (plram == pic->P18map.PREINC2) {
            pic->lram = VALID_ADDR(afsr2 + 1);
            pic->ram[pic->lram] = (*pic->P18map.PREINC2);
            (*pic->P18map.FSR2H) = ((afsr2 + 1) & 0xFF00) >> 8;
            (*pic->P18map.FSR2L) = (afsr2 + 1) & 0x00FF;
        }
        if (plram == pic->P18map.PLUSW2) {
            pic->lram = VALID_ADDR(afsr2 + ((char)(*pic->P18map.WREG)));
            pic->ram[pic->lram] = (*pic->P18map.PLUSW2);
        }
        ...
        if (plram == pic->P18map.TOSL) {
            if (((*pic->P18map.STKPTR) & 0x1F) > 0)
                pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] =
                    (pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] & 0x1FFF00) | (*pic->P18map.TOSL);
        }
        if (plram == pic->P18map.TOSH) {
            if (((*pic->P18map.STKPTR) & 0x1F) > 0)
                pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] =
                    (pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] & 0x1F00FF) | ((*pic->P18map.TOSH) << 8);
        }
        if (plram == pic->P18map.TOSU) {
            if (((*pic->P18map.STKPTR) & 0x1F) > 0)
                pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] =
                    (pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1] & 0x00FFFF) | (((*pic->P18map.TOSU) & 0x1F) << 16);
        }

        if (plram == pic->P18map.LATA) {
            (*pic->P18map.PORTA) =
                ((*pic->P18map.PORTA) & (*pic->P18map.TRISA)) | ((*pic->P18map.LATA) & (~(*pic->P18map.TRISA)));
        }
        if (plram == pic->P18map.LATB) {
            (*pic->P18map.PORTB) =
                ((*pic->P18map.PORTB) & (*pic->P18map.TRISB)) | ((*pic->P18map.LATB) & (~(*pic->P18map.TRISB)));
        }
        if (plram == pic->P18map.LATC) {
            (*pic->P18map.PORTC) =
                ((*pic->P18map.PORTC) & (*pic->P18map.TRISC)) | ((*pic->P18map.LATC) & (~(*pic->P18map.TRISC)));
        }
        if (plram == pic->P18map.LATD) {
            (*pic->P18map.PORTD) =
                ((*pic->P18map.PORTD) & (*pic->P18map.TRISD)) | ((*pic->P18map.LATD) & (~(*pic->P18map.TRISD)));
        }
        if (plram == pic->P18map.LATE) {
            (*pic->P18map.PORTE) =
                ((*pic->P18map.PORTE) & (*pic->P18map.TRISE)) | ((*pic->P18map.LATE) & (~(*pic->P18map.TRISE)));
        }
