...
void pic_decode_16(_pic *pic) {
  ...
  case 0x0000:
    ...
    case 0x0700:
      ...
      if ((0xF0 &
           ((0x0F & pic->w) + (0x0F & pic->ram[bank | (opc & 0x007F)]))) != 0)
        *status |= 0x02;
      else
        *status &= ~0x02;
  ...
  case 0x2000:
    ...
    case 0x0000:
      ...
      pic->stack[0] = pic->pc;
      pic->jpc =
          ((pic->ram[bank | (sfr_addr(pic->P16map.PCLATH) & 0x007F)] & 0x18)
           << 8) |
          (opc & 0x07FF);
      pic->s2 = 1;
      break;
    case 0x0800:
      ...
      if (pic->print)
        printf("GOTO %#05X\n", opc & 0x07FF);
#ifdef ICSPDBG
      if (pic->getconf(CFG_DEBUG) &&
          ((((*pic->P16map.ICKBUG)) & 0x80) == 0x80)) // DEBUG ON
  ...
  if ((bank | (sfr_addr(pic->P16map.PCL) & 0x007F)) == pic->lram) {
    pic->pc = ((pic->ram[bank | (sfr_addr(pic->P16map.PCLATH) & 0x007F)] & 0x1F)
               << 8) |
  ...
  if ((pic->rram != 0x8000) && (pic->print))
    printf("mem read  %#06X: %10s= %#06X\n", pic->rram,
           getFSRname_16(pic->rram), pic->ram[pic->rram]);
  if ((pic->lram != 0x8000) && (pic->print))
    printf("mem write %#06X: %10s= %#06X\n", pic->lram,
           getFSRname_16(pic->lram), pic->ram[pic->lram]);

  // latchs
  if ((pic->lram == sfr_addr(pic->P16map.PORTA)) ||
      (pic->lram == (sfr_addr(pic->P16map.PORTA) + 0x100))) {
    pic->p16latch[0] = (*pic->P16map.PORTA);
  }

  if ((pic->lram == sfr_addr(pic->P16map.PORTB)) ||
      (pic->lram == (sfr_addr(pic->P16map.PORTB) + 0x100))) {
    pic->p16latch[1] = (*pic->P16map.PORTB);
  }

  if (pic->P16map.PORTC) {
    if ((pic->lram == sfr_addr(pic->P16map.PORTC)) ||
        (pic->lram == (sfr_addr(pic->P16map.PORTC) + 0x100))) {
      pic->p16latch[2] = (*pic->P16map.PORTC);
    }
  }

  if (pic->P16map.PORTD) {
    if ((pic->lram == sfr_addr(pic->P16map.PORTD)) ||
        (pic->lram == (sfr_addr(pic->P16map.PORTD) + 0x100))) {
      pic->p16latch[3] = (*pic->P16map.PORTD);
    }
  }

  if (pic->P16map.PORTE) {
    if ((pic->lram == sfr_addr(pic->P16map.PORTE)) ||
        (pic->lram == (sfr_addr(pic->P16map.PORTE) + 0x100))) {
