...
pic_decode_16(void)
{
 ...
 unsigned short afsr = (((*pic->P16map.STATUS) & 0x0080) << 1) | pic->ram[bank | (sfr_addr (pic->P16map.FSR) &0x007F)];
   ...
   switch (opc & 0x0800)
    ...
    case 0x0000:
     ...
     for (temp = 7; temp > 0; temp--)
      pic->stack[temp] = pic->stack[temp - 1];
     pic->stack[0] = pic->pc;
     pic->jpc = ((pic->ram[bank | (sfr_addr (pic->P16map.PCLATH) & 0x007F)]&0x18) << 8) | (opc & 0x07FF);
     pic->s2 = 1;
     break;
    case 0x0800:
 ...
 if ((bank | (sfr_addr (pic->P16map.PCL) & 0x007F)) == pic->lram)
  {
   pic->pc = ((pic->ram[bank | (sfr_addr (pic->P16map.PCLATH) & 0x007F)]&0x1F) << 8) | pic->ram[bank | (sfr_addr (pic->P16map.PCL)& 0x007F)];
  }
