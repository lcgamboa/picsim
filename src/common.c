...
unsigned short *memsets(unsigned short *mem, unsigned short value,
                        unsigned long size) {
  unsigned long i;
  for (i = 0; i < size; i++)
    mem[i] = value;
...
int pic_init(_pic *pic, int processor, const char *fname, int leeprom,
             float freq) {
  int i;
  int retcode = 0;
...
int pic_getMCLRE(_pic *pic) { return pic->getconf(pic, CFG_MCLR); }
...
int pic_set_apin(_pic *pic, unsigned char pin, float value) {
  ...
  if (pin && ((pin - 1) < pic->PINCOUNT)) {
    ...
    if ((pic->pins[(pin - 1)].dir == PD_IN) &&
        ((pic->pins[(pin - 1)].ptype == PT_ANALOG) ||
         (pic->pins[(pin - 1)].ptype == PT_ANAREF))) {
