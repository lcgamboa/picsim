...
void PIC16F648A_reset(_pic* pic) {
...
void PIC16F648A_periferic(_pic* pic) {
    p16_int_pin(pic);
    p16_int_portb(pic);
    p16_tmr0(pic);
    p16_wdt(pic);
...
int PIC16F648A_getconf(_pic* pic, unsigned int cfg) {
    switch (cfg) {
        case CFG_MCLR:
...
void PIC16F648A_disable_debug(_pic* pic) {
    pic->config[0] |= 0x0800;
}

void PIC16F648A_stop(_pic* pic) {
    p16_uart_stop(pic, 0);
}
...
void PIC16F648A_start(_pic* pic) {
    ...
    pic->periferic = PIC16F648A_periferic;
    pic->stop = PIC16F648A_stop;
    pic->disable_debug = PIC16F648A_disable_debug;

    p16_uart_start(pic, 0);
}

init_pic(PIC16F648A, P16, 0x1100);
