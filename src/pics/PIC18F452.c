...
void PIC18F452_periferic(_pic* pic) {
    p18_mssp(pic);
    p18_adc_2(pic);
    p18_int_pin(pic);
    p18_int_portb(pic);
...
int PIC18F452_getconf(_pic* pic, unsigned int cfg) {
    switch (cfg) {
        case CFG_MCLR:
            return 1;
            break;
...
void PIC18F452_disable_debug(_pic* pic) {
    pic->config[3] |= 0x0080;
}

void PIC18F452_stop(_pic* pic) {
    p18_uart_stop(pic, 0);
}
...
void PIC18F452_start(_pic* pic) {
    ...
    pic->stop = PIC18F452_stop;
    pic->disable_debug = PIC18F452_disable_debug;
