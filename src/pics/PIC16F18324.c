...
PIC16F18324_reset(void)
{
 ...
 pic->pins[ 7].port = pic->P16Emap.PORTC;pic->pins[ 7].pord = 2;
 pic->pins[ 8].port = pic->P16Emap.PORTC;pic->pins[ 8].pord = 1;
 pic->pins[ 9].port = pic->P16Emap.PORTC;pic->pins[ 9].pord = 0;
 pic->pins[10].port = pic->P16Emap.PORTA;pic->pins[10].pord = 2;
 pic->pins[11].port = pic->P16Emap.PORTA;pic->pins[11].pord = 1;
 pic->pins[12].port = pic->P16Emap.PORTA;pic->pins[12].pord = 0;
 pic->pins[13].port = P_VSS    ;pic->pins[13].pord = -1;
...
int
PIC16F18324_getconf(unsigned int cfg)
{
 switch (cfg)
  {
  case CFG_MCLR:
...
void
PIC16F18324_stop(void)
{
 p16e_uart_stop (0);
}

void
PIC16F18324_start(void)
{
 pic->ROMSIZE = 4096;
 pic->EEPROMSIZE = 256;
 pic->RAMSIZE = 4096;
