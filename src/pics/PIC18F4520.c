...
void PIC18F4520_reset(_pic* pic) {
...
void PIC18F4520_periferic(_pic* pic) {
    p18_mssp(pic);
    p18_adc(pic);
    p18_int_pin(pic);
    p18_int_portb(pic);
...
int PIC18F4520_getconf(_pic* pic, unsigned int cfg) {
    switch (cfg) {
        case CFG_MCLR:
...
void PIC18F4520_disable_debug(_pic* pic) {
    pic->config[3] |= 0x0080;
}

void PIC18F4520_stop(_pic* pic) {
    p18_uart_stop(pic, 0);
}
