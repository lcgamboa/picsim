...
20 - Vdd
21 - RB0 AN12 INT
22 - RB1 AN10 P1C C12IN3-
23 - RB2 AN8 P1B
24 - RB3 AN9 PGM C12IN2-
25 - RB4 AN11 P1D
26 - RB5 AN13 T1G
27 - RB6 ICSPCLK
28 - RB7 ICSPDAT
...
void PIC16F886_reset(_pic* pic) {
    ...
    pic->pins[7].pord = -1;
    pic->pins[8].port = pic->P16map.PORTA;
    pic->pins[8].pord = 7;
    pic->pins[9].port = pic->P16map.PORTA;
    pic->pins[9].pord = 6;
    pic->pins[10].port = pic->P16map.PORTC;
    pic->pins[10].pord = 0;
    pic->pins[11].port = pic->P16map.PORTC;
    pic->pins[11].pord = 1;
    pic->pins[12].port = pic->P16map.PORTC;
    pic->pins[12].pord = 2;
    pic->pins[13].port = pic->P16map.PORTC;
    pic->pins[13].pord = 3;
    ...
    pic->pins[19].pord = -1;
    pic->pins[19].value = 1;
    pic->pins[20].port = pic->P16map.PORTB;
    pic->pins[20].pord = 0;
    pic->pins[21].port = pic->P16map.PORTB;
    pic->pins[21].pord = 1;
    pic->pins[22].port = pic->P16map.PORTB;
    pic->pins[22].pord = 2;
    pic->pins[23].port = pic->P16map.PORTB;
    pic->pins[23].pord = 3;
    pic->pins[24].port = pic->P16map.PORTB;
    pic->pins[24].pord = 4;
    pic->pins[25].port = pic->P16map.PORTB;
    pic->pins[25].pord = 5;
    pic->pins[26].port = pic->P16map.PORTB;
    pic->pins[26].pord = 6;
    pic->pins[27].port = pic->P16map.PORTB;
    pic->pins[27].pord = 7;
...
void PIC16F886_periferic(_pic* pic) {
    p16_mssp(pic);
    p16_adc_2(pic);
    p16_int_pin(pic);
    p16_int_portb_2(pic);
...
int PIC16F886_getconf(_pic* pic, unsigned int cfg) {
    switch (cfg) {
        case CFG_MCLR:
...
void PIC16F886_disable_debug(_pic* pic) {
    pic->config[0] |= 0x2000;
}

void PIC16F886_stop(_pic* pic) {
    p16_uart_stop(pic, 0);
}
...
void PIC16F886_start(_pic* pic) {
    ...
    pic->USARTCOUNT = 1;
    pic->WDT_MS = 1;
    pic->reset = PIC16F886_reset;
    pic->mmap = PIC16F886_map;
    pic->getconf = PIC16F886_getconf;
    pic->periferic = PIC16F886_periferic;
    pic->stop = PIC16F886_stop;
    pic->disable_debug = PIC16F886_disable_debug;

    p16_uart_start(pic, 0);
}

init_pic(PIC16F886, P16, 0x2060);
