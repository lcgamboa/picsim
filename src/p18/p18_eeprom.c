...
void p18_eeprom_rst(_pic *pic) { pic->ee_wr = 0; }
  ...
  if (((*pic->P18map.EECON1) & 0x03) != 0x00) {
    if (((*pic->P18map.EECON1) & 0x01) == 0x01) // RD
    {
      ...
      /*
            ...
            {
               (*pic->P18map.EEDATA)=
         (pic->prog[(pic->ram[P18_EEADRH]<<8)|(*pic->P18map.EEADR)])&0x00FF;
               pic->ram[P18_EEDATAH]=
         ((pic->prog[(pic->ram[P18_EEADRH]<<8)|(*pic->P18map.EEADR)])&0xFF00)>>8;
            }
       */
      ...
      if (pic->ee_wr == 1) {
        ...
        /*
                ...
                {
                  pic->prog[(pic->ram[P18_EEADRH]<<8)|(*pic->P18map.EEADR)] =
           (pic->ram[P18_EEDATAH]<<8) | (*pic->P18map.EEDATA);
  ...
  if (((*pic->P18map.NVMCON1) & 0x03) != 0x00) {
    if (((*pic->P18map.NVMCON1) & 0x01) == 0x01) // RD
    {
      if (((*pic->P18map.NVMCON1) & 0x80) == 0) {
        (*pic->P18map.NVMDAT) =
            pic->eeprom[((*pic->P18map.NVMADRH) << 8) | (*pic->P18map.NVMADRL)];
      }
      /*
            else
            {
