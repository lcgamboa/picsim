   ...
   switch ((*pic->P18map.SSPCON1) & 0x0F)
     ...
     if (pic->ssp_bit)
      {
       if (pic->ssp_sck == 1)
        {
         pic_wr_pin18 (pic->sdo, (((*pic->P18map.SSPBUF) & (1 << ((pic->ssp_bit - 1)))) > 0));
    ...
    case 0x05://SPI Slave mode, clock = SCK pin. SS pin control disabled.
     ...
     if ((pic->ssp_scka == 0)&&(pic->ssp_sck == 1)) //CKP =0 CKE =0     //coloca saida na borda de subida
      {
       pic_wr_pin18 (pic->sdo, (((*pic->P18map.SSPBUF) & (1 << (7 - pic->ssp_bit))) > 0));
      }
       ...
       if (((*pic->P18map.SSPCON2) & 0x01)) //start
        {
         ...
         pic_dir_pin18 (pic->sdi, PD_OUT);
         pic_wr_pin18 (pic->sdi, 1);
         pic_wr_pin18 (pic->sck, 1);
         pic->ssp_ck |= 0x01;
         //printf("I2C_start 1\n");
        }
     ...
     if (pic->ssp_sck == ((((*pic->P18map.SSPADD) + 1) / 2)))
      {
       ...
       else if (((*pic->P18map.SSPCON2) & 0x02)&&(pic->ssp_ck & 0x02)) //restart
        {
       ...
       else if (((*pic->P18map.SSPCON2) & 0x04)&&(pic->ssp_ck & 0x04)) //stop
        {
     ...
     if (pic->ssp_sck > (*pic->P18map.SSPADD))
      {
       ...
       if ((((*pic->P18map.SSPSTAT) & 0x04))&&(pic->ssp_bit <= 10))
        {
         ...
         if ((pic->pins[pic->sck - 1].value == 0)&&(pic->ssp_bit <= 8))
          {
           pic_wr_pin18 (pic->sdi, ((*pic->P18map.SSPBUF) & (0x01 << (7 - pic->ssp_bit))) > 0);
           pic->ssp_bit++;
           if (pic->ssp_bit == 9)
            {
             pic_dir_pin18 (pic->sdi, PD_IN);
         ...
         pic_wr_pin18 (pic->sck, !pic->pins[pic->sck - 1].value);
       ...
       //read 
       if ((((*pic->P18map.SSPCON2) & 0x08) || ((*pic->P18map.SSPCON2)& 0x10))&&(pic->ssp_bit <= 10))
        {
         ...
         if ((pic->pins[pic->sck - 1].value == 1)&&(pic->ssp_bit <= 8))
          ...
          if (((*pic->P18map.SSPCON2)& 0x10)&&(pic->pins[pic->sck - 1].value == 0)&&(pic->ssp_bit > 8))
          {
           pic_dir_pin18 (pic->sdi, PD_OUT);
           pic_wr_pin18 (pic->sdi, ((*pic->P16map.SSPCON2)&0x20) > 0);
           pic->ssp_bit++;
          }
 ...
 if (((*pic->P18map.SSP1CON1) & 0x20) == 0x20) //SSPEN
  {
   ...
   switch ((*pic->P18map.SSP1CON1) & 0x0F)
     ...
     if (pic->ssp_bit)
      {
       if (pic->ssp_sck == 1)
        {
         pic_wr_pin18 (pic->sdo, (((*pic->P18map.SSP1BUF) & (1 << ((pic->ssp_bit - 1)))) > 0));
         pic_wr_pin18 (pic->sck, 1);
        }
       else if (pic->ssp_sck == 2)
        {
       ...
       if(!pic->ssp_bit)(*pic->P18map.SSP1STAT) |= 0x01; //BF
      }
    ...
    case 0x05://SPI Slave mode, clock = SCK pin. SS pin control disabled.
     ...
     if ((pic->ssp_scka == 0)&&(pic->ssp_sck == 1)) //CKP =0 CKE =0     //coloca saida na borda de subida
      {
       pic_wr_pin18 (pic->sdo, (((*pic->P18map.SSP1BUF) & (1 << (7 - pic->ssp_bit))) > 0));
      }
    ...
    case 0x08://I2C Master mode, clock = FOSC /(4 * (SSP11ADD + 1))
     ...
     if (pic->ssp_sck == ((((*pic->P18map.SSP1ADD) + 1) / 2)))
      {
       ...
       else if (((*pic->P18map.SSP1CON2) & 0x02)&&(pic->ssp_ck & 0x02)) //restart
        {
       ...
       else if (((*pic->P18map.SSP1CON2) & 0x04)&&(pic->ssp_ck & 0x02)) //stop
        {
     ...
     if (!((*pic->P18map.SSP1CON2) & 0x08)) pic->ssp_scka = 0;
     ...
     if (pic->ssp_sck > (*pic->P18map.SSP1ADD))
      {
       ...
       if ((((*pic->P18map.SSP1STAT) & 0x04))&&(pic->ssp_bit <= 10))
        {
         ...
         if ((pic->pins[pic->sck - 1].value == 0)&&(pic->ssp_bit <= 8))
          {
           pic_wr_pin18 (pic->sdi, ((*pic->P18map.SSP1BUF) & (0x01 << (7 - pic->ssp_bit))) > 0);
           pic->ssp_bit++;
           if (pic->ssp_bit == 9)
            {
             pic_dir_pin18 (pic->sdi, PD_IN);
       ...
       //read 
       if ((((*pic->P18map.SSP1CON2) & 0x08) || ((*pic->P18map.SSP1CON2)& 0x10))&&(pic->ssp_bit <= 10))
        {

         if ((pic->pins[pic->sck - 1].value == 1)&&(pic->ssp_bit <= 8))
          {
           pic->ssp_bit++;   
           (*pic->P18map.SSP1BUF) |= (pic->pins[pic->sdi - 1].value << (8 - pic->ssp_bit));
           if (pic->ssp_bit == 8)
            {
          ...
          if (((*pic->P18map.SSP1CON2)& 0x10)&&(pic->pins[pic->sck - 1].value == 0)&&(pic->ssp_bit > 8))
          {
           pic_dir_pin18 (pic->sdi, PD_OUT);
           pic_wr_pin18 (pic->sdi, ((*pic->P16map.SSPCON2)&0x20) > 0);
           pic->ssp_bit++;
