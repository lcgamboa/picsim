       ...
       if ((((*pic->P18map.SSPCON2) & 0x08) || ((*pic->P18map.SSPCON2)& 0x10))&&(pic->ssp_bit <= 9))
        {
         ...
         pic->ssp_ck ^= 0x01;
         pic_wr_pin18 (pic->sck, pic->ssp_ck);
         //printf("sck 2 =%i\n", pic->pins[pic->sck-1].value);
         ...
         if (((*pic->P18map.SSPCON2)& 0x10)&&(pic->pins[pic->sck - 1].value == 1)&&(pic->ssp_bit > 8))
          {
           pic_dir_pin18 (pic->sdi, PD_OUT);
           pic_wr_pin18 (pic->sdi, ((*pic->P18map.SSPCON2)&0x20) > 0);
           (*pic->P18map.PIR1) |= 0x08; //SSPIF
           pic->ssp_bit++;
 ...
 if (((*pic->P18map.SSP1CON1) & 0x20) == 0x20) //SSPEN
  {
   ...
   switch ((*pic->P18map.SSP1CON1) & 0x0F)
    ...
    case 0x08://I2C Master mode, clock = FOSC /(4 * (SSP11ADD + 1))
     ...
     if (pic->ssp_sck == (((*pic->P18map.SSP1ADD) + 1 / 2) - 1))
      {
     ...
     if (pic->ssp_sck > (*pic->P18map.SSP1ADD))
      {
       ...
       //write
       if ((((*pic->P18map.SSP1STAT) & 0x04))&&(pic->ssp_bit <= 9))
        {
         pic_wr_pin18 (pic->sck, !pic->pins[pic->sck - 1].value);
       ...
       //read 
       if ((((*pic->P18map.SSP1CON2) & 0x08) || ((*pic->P18map.SSP1CON2)& 0x10))&&(pic->ssp_bit <= 9))
        {
         //printf("sck 1 =%i\n", pic->pins[pic->sck-1].value);
         pic->ssp_ck ^= 0x01;
         pic_wr_pin18 (pic->sck, pic->ssp_ck);
         ...
         if (((*pic->P18map.SSP1CON2)& 0x10)&&(pic->pins[pic->sck - 1].value == 1)&&(pic->ssp_bit > 8))
          {
           pic_dir_pin18 (pic->sdi, PD_OUT);
           pic_wr_pin18 (pic->sdi, ((*pic->P18map.SSP1CON2)&0x20) > 0);
           (*pic->P18map.PIR1) |= 0x08; //SSP1IF
           pic->ssp_bit++;
