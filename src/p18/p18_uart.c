...
void p18_uart_rst(_pic *pic, int nser) {
  if (nser == 0) {
    ...
    pic->serial[0].serial_TRIS_RX =
        &pic->ram[sfr_addr(pic->pins[pic->usart_rx[0] - 1].port) + 0x12];
...
void p18_uart_rst_3(_pic *pic, int nser) {
  ...
  if (nser == 0) {
    ...
    pic->serial[0].serial_TRIS_RX =
        &pic->ram[sfr_addr(pic->pins[pic->usart_rx[0] - 1].port) - 0x05];
    pic->serial[0].serial_TRIS_RX_MASK =
        (0x01 << pic->pins[pic->usart_rx[0] - 1].pord);
    pic->serial[0].s_open = 1;
    bb_uart_rst(&pic->serial[0].bbuart);
    ...
    pic->serial[1].serial_TRIS_RX =
        &pic->ram[sfr_addr(pic->pins[pic->usart_rx[1] - 1].port) - 0x05];
    pic->serial[1].serial_TRIS_RX_MASK =
        (0x01 << pic->pins[pic->usart_rx[1] - 1].pord);
