...
periferic16_step_in(void)
{
 ...
 if (pic->P16map.SSPCON)//(pic->processor == P16F877) || (pic->processor == P16F877A) || (pic->processor == P16F777))
  {
   //MSSP
     ...
     switch (((*pic->P16map.SSPCON) & 0x0F))
       ...
       if (pic->ssp_bit)
        {
         if (pic->ssp_sck == 1)
          {
           pic_wr_pin16 (pic->sdo, (((*pic->P16map.SSPBUF) & (1 << ((pic->ssp_bit - 1)))) > 0));
      ...
      case 0x05://SPI Slave mode, clock = SCK pin. SS pin control disabled.
       ...
       if ((pic->ssp_scka == 0)&&(pic->ssp_sck == 1)) //CKP =0 CKE =0     //coloca saida na borda de subida
        {
         pic_wr_pin16 (pic->sdo, (((*pic->P16map.SSPBUF) & (1 << (7 - pic->ssp_bit))) > 0));
        }
       ...
       if (pic->ssp_sck > (*pic->P16map.SSPADD))
        {
         ...
         //read 
         if ((((*pic->P16map.SSPCON2) & 0x08) || ((*pic->P16map.SSPCON2)& 0x10))&&(pic->ssp_bit <= 9))
          {
           ...
           if ((pic->pins[pic->sck - 1].value == 1)&&(pic->ssp_bit <= 8))
            {
             (*pic->P16map.SSPBUF) |= (pic->pins[pic->sdi - 1].value << (8 - pic->ssp_bit));
             if (pic->ssp_bit == 8)
              {
       ...
       if ((*pic->P16map.TMR2) == (*pic->P16map.PR2))
        {
         ...
         if ((pic->CCPCOUNT >= 1)&&(pic->ccp[0] > 0)&&((*pic->P16map.CCP1CON)& 0x0C) == 0x0C)
          {
         ...
         if ((pic->CCPCOUNT >= 2)&&(pic->ccp[1] > 0)&&((*pic->P16map.CCP2CON)& 0x0C) == 0x0C)
          {
         ...
         if ((pic->CCPCOUNT >= 3)&&(pic->ccp[2] > 0)&&((*pic->P16map.CCP3CON)& 0x0C) == 0x0C)
          {
