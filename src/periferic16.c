  ...
  if (pic->getconf(CFG_DEBUG)) // DEBUG ON
  {
    ...
    if (((*pic->P16map.ICKBUG) & 0x80) == 0) {
      ...
      if (((pic->ocd_pgca == 1) && (pic->ocd_pgc == 0)) || // HardWare
          ((pic->pc ==
            (((((*pic->P16map.ICKBUG) & 0x1F) << 8) | (*pic->P16map.BIGBUG)) +
             1)) &&
           (pic->dbg > 3)) || // Break Point
          ((((*pic->P16map.ICKBUG) & 0x20) == 0x20) &&
           (pic->sstep == 3))) // step
      {
        ...
        (*pic->P16map.ICKBUG) |= 0x80;
        (*pic->P16map.ICKBUG) =
            ((*pic->P16map.ICKBUG) & 0xE0) | ((pic->pc & 0x1F00) >> 8);
    ...
    for (i = 0; i < pic->PINCOUNT; i++) {
      if (pic->pins[i].pord >= 0)
        ...
        case PD_IN:
          ...
          port = sfr_addr(pic->pins[i].port);
          pic->pins[i].lvalue =
              ((pic->p16latch[port - sfr_addr(pic->P16map.PORTA)] & val) !=
               0); // latch
