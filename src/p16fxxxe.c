...
void pic_decode_16E(_pic *pic) {
  unsigned short temp;
  unsigned short opc;
  unsigned short bank = ((*pic->P16Emap.BSR) & 0x003F) << 7;
  unsigned char *status =
      &pic->ram[bank | (sfr_addr(pic->P16Emap.STATUS) & 0x007F)];
  ...
  case 0x0000:
    ...
    case 0x0000:
      ...
      case 0x0000:
        ...
        case 0x0000:
          ...
          break;
        case 0x0001:
          // RESET    -     Software device Reset              1    00 0000 0000
          // 0001
          if (pic->print)
            printf("RESET\n");
          pic_reset(pic, 0);
        ...
        case 0x0009:
          ...
          break;
        case 0x000A:
          // CALLW    -     Call Subroutine with W             2    00 0000 0000
          // 1010
          if (pic->print)
            printf("CALLW\n");
          ...
          pic->stack[0] = pic->pc;
          pic->jpc =
              (((*pic->P16Emap.PCLATH) & 0x1F) << 8) | (*pic->P16Emap.WREG);
          pic->s2 = 1;
          break;
        case 0x000B:
          // BRW      -     Relative Branch with W             2    00 0000 0000
          // 1011
          if (pic->print)
            printf("BRW\n");
          // TODO P16E HW DEBUG support
          /*
                  if(((pic->config[0] & 0x0800) ==
          0)&&(((pic->ram[P16E_ICKBUG])&0x80)== 0x80) )//DEBUG ON
          //	  pic->jpc=((0x18)<<8)|(opc & 0x07FF);
                    pic->jpc=(pic->ROMSIZE-1)&(((0xF8)<<8)|(opc & 0x07FF));
                 else
           */
          pic->jpc =
              (pic->pc + (*pic->P16Emap.WREG)) & ((pic->ROMSIZE << 1) - 1);
          pic->s2 = 1;
          break;
        case 0x0010 ... 0x0017:
          // MOVIW    n,m   Move Indirect FSRn to W with       1    00 0000 0001
          // 0nmm   Z
          //                pre/post inc/dec modifier, mm
          if (pic->print)
            printf("MOVIW FSR%i, %i\n", (opc & 0x0004) != 0, opc & 0x0003);
          ...
          break;
        case 0x0018 ... 0x001F:
          // MOVWI    n,m   Move W to Indirect FSRn with       1    00 0000 0001
          // 1nmm
          //                pre/post inc/dec modifier, mm
          if (pic->print)
            printf("MOVWI FSR%i, %i\n", (opc & 0x0004) != 0, opc & 0x0003);
        ...
        case 0x0064:
          ...
          *status |= 0x10;
          break;
        case 0x0065 ... 0x0067:
          // TRIS     f     Load TRIS register with W          1    00 0000 0110
          // 01ff
          if (pic->print)
            printf("TRIS %#04X\n", opc & 0x0007);
          switch (opc & 0x0003) {
          ...
          break;
        case 0x006A:
          // OPTION   -     Load OPTION_REG register with W    1    00 0000 0110
          // 0010
          if (pic->print)
            printf("OPTION\n");
    ...
    case 0x0200:
      ...
      if ((0xF0 & ((0x0F & (~(*pic->P16Emap.WREG))) + 1 +
                   (0x0F & pic->ram[bank | (opc & 0x007F)]))) != 0)
        *status |= 0x02;
      else
        *status &= ~0x02;
  ...
  case 0x1000:
    ...
    case 0x0800:
      ...
      if (pic->print)
        printf("BTFSC %#04X,%d\n", opc & 0x007F, (opc & 0x0380) >> 7);
      if ((pic->ram[bank | (opc & 0x007F)] & (0x01 << ((opc & 0x0380) >> 7))) ==
          0) {
        pic->jpc = pic->pc + 1;
        pic->s2 = 1;
      }
  ...
  case 0x2000:
    ...
    case 0x0000:
      ...
      pic->stack[0] = pic->pc;
      pic->jpc =
          ((pic->ram[bank | (sfr_addr(pic->P16Emap.PCLATH) & 0x007F)] & 0x18)
           << 8) |
          (opc & 0x07FF);
      pic->s2 = 1;
      break;
  ...
  case 0x3000:
    ...
    case 0x0100:
      ...
      case 0x0080:
        ...
        break;
      case 0x0000:
        // ADDFSR   n,k   Add Literal k to FSRn              1    11 0001 0nkk
        // kkkk
        jrange = (((short)((opc & 0x003F) << 10)) / 1024);
        if (pic->print)
          printf("ADDFSR FSR%i ,%i\n", (opc & 0x0040) != 0, jrange);
        if (opc & 0x0040) // FSR1
        {
          afsr[1] += jrange;
          (*pic->P16Emap.FSR1H) = (afsr[1] & 0xFF00) >> 8;
    ...
    case 0x0200:
    case 0x0300:
      //         BRA      k     Relative Branch                    2    11 001k
      //         kkkk kkkk
    ...
    case 0x0400:
      ...
      break;
    case 0x0500:
      // LSLF     f,d   Logical Left Shift                 1    11 0101 dfff
      // ffff     C, Z        2
      if (pic->print)
        printf("LSLF %#04X,%d\n", opc & 0x007F, (opc & 0x0080) >> 7);
      temp = (pic->ram[bank | (opc & 0x007F)]);
      ...
      break;
    case 0x0600:
      // LSRF     f,d   Logical Right Shift                1    11 0110 dfff
      // ffff     C, Z        2
      if (pic->print)
        printf("LSRF %#04X,%d\n", opc & 0x007F, (opc & 0x0080) >> 7);
      temp = (pic->ram[bank | (opc & 0x007F)]);
      ...
      break;
    case 0x0700:
      // ASRF     f,d   Arithmetic Right Shift             1    11 0111 dfff
      // ffff     C, Z        2
      if (pic->print)
        printf("ASRF %#04X,%d\n", opc & 0x007F, (opc & 0x0080) >> 7);
      temp = (pic->ram[bank | (opc & 0x007F)]);
    ...
    case 0x0A00:
      ...
      break;
    case 0x0B00:
      // SUBWFB   f,d   Subtract with Borrow W from f      1    11 1011 dfff
      // ffff     C, DC, Z    2
      if (pic->print)
        printf("SUBWFB %#04X,%d\n", opc & 0x007F, (opc & 0x0080) >> 7);
      if ((0xF0 & ((0x0F & (~(*pic->P16Emap.WREG))) +
                   (!((*pic->P16Emap.STATUS) & 0x01)) + 1 +
                   (0x0F & pic->ram[bank | (opc & 0x007F)]))) != 0)
        *status |= 0x02;
      else
        *status &= ~0x02;
      ...
      temp =
          (~((*pic->P16Emap.WREG) + (!((*pic->P16Emap.STATUS) & 0x01))) + 1) +
          pic->ram[bank | (opc & 0x007F)];
      ...
      break;
    case 0x0C00:
      // SUBLW   k  	Subtract W from literal      1     111100 kkkkkkkk
      // C,DC,Z
      if (pic->print)
        printf("SUBLW %#04X\n", opc & 0x00FF);
      ...
      break;
    case 0x0D00:
      // ADDWFC   f,d   Add W and f and Carry                       1    11 1101
      // dfff ffff     C, DC, Z    2
      if (pic->print)
        printf("ADDWFC %#04X,%d\n", opc & 0x007F, (opc & 0x0080) >> 7);

      if ((0xF0 & ((0x0F & (*pic->P16Emap.WREG)) +
                   (0x0F & pic->ram[bank | (opc & 0x007F)]) +
                   ((*pic->P16Emap.STATUS) & 0x01))) != 0)
        *status |= 0x02;
      else
        *status &= ~0x02;

      temp = (*pic->P16Emap.WREG) + pic->ram[bank | (opc & 0x007F)] +
             ((*pic->P16Emap.STATUS) & 0x01);
      ...
      break;
    case 0x0E00:
      // ADDLW   k  	Add literal and W            1     111110 kkkkkkkk
      // C,DC,Z
      if (pic->print)
        printf("ADDLW %#04X\n", opc & 0x00FF);
    ...
    case 0x0F00:
      switch (opc & 0x0080) {
      case 0x0000:
        // MOVIW    k[n]  Move INDFn to W, Indexed Indirect  1    11 1111 0nkk
        // kkkk   Z
        jrange = (((short)((opc & 0x003F) << 10)) / 1024);
        if (pic->print)
          printf("MOVIW %i[FSR%i]\n", jrange, (opc & 0x0040) != 0);
        pic->lram = sfr_addr(pic->P16Emap.WREG);
        if (opc & 0x0040) // INDF1
        {
          afsr[1] += jrange;
  ...
  if (pic->lram != 0x8000) {
    ...
    if (plram == pic->P16Emap.STKPTR) {
      if (((*pic->P16Emap.STKPTR) & 0x1F) > 0) {
        (*pic->P16Emap.TOSL) =
            ((pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1]) & 0x0000FF);
        (*pic->P16Emap.TOSH) =
            ((pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1]) & 0x00FF00) >> 8;
      } else {
        (*pic->P16Emap.TOSL) = 0;
        (*pic->P16Emap.TOSH) = 0;
      }
    ...
    if (plram == pic->P16Emap.TOSL) {
      if (((*pic->P16Emap.STKPTR) & 0x1F) > 0)
        pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1] =
            (pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1] & 0x1FFF00) |
            (*pic->P16Emap.TOSL);
    }
    if (plram == pic->P16Emap.TOSH) {
      if (((*pic->P16Emap.STKPTR) & 0x1F) > 0)
        pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1] =
            (pic->stack[((*pic->P16Emap.STKPTR) & 0x1F) - 1] & 0x1F00FF) |
            ((*pic->P16Emap.TOSH) << 8);
    }
    ...
    if (plram == pic->P16Emap.LATA) {
      (*pic->P16Emap.PORTA) = ((*pic->P16Emap.PORTA) & (*pic->P16Emap.TRISA)) |
                              ((*pic->P16Emap.LATA) & (~(*pic->P16Emap.TRISA)));
    }
    if (plram == pic->P16Emap.LATB) {
      (*pic->P16Emap.PORTB) = ((*pic->P16Emap.PORTB) & (*pic->P16Emap.TRISB)) |
                              ((*pic->P16Emap.LATB) & (~(*pic->P16Emap.TRISB)));
    }
    if (plram == pic->P16Emap.LATC) {
      (*pic->P16Emap.PORTC) = ((*pic->P16Emap.PORTC) & (*pic->P16Emap.TRISC)) |
                              ((*pic->P16Emap.LATC) & (~(*pic->P16Emap.TRISC)));
    }
    if (plram == pic->P16Emap.LATD) {
      (*pic->P16Emap.PORTD) = ((*pic->P16Emap.PORTD) & (*pic->P16Emap.TRISD)) |
                              ((*pic->P16Emap.LATD) & (~(*pic->P16Emap.TRISD)));
    }
    if (plram == pic->P16Emap.LATE) {
      (*pic->P16Emap.PORTE) = ((*pic->P16Emap.PORTE) & (*pic->P16Emap.TRISE)) |
                              ((*pic->P16Emap.LATE) & (~(*pic->P16Emap.TRISE)));
    }
    ...
    if ((pic->rram != 0x8000)) {
      if (pic->rram <= 0x0FFF) {
        printf("mem read  %#06X: %10s= %#06X\n", pic->rram,
               getFSRname_16E2(pic->rram), pic->ram[pic->rram]);
      } else {
        if (pic->rram >= 0x8000) {
    ...
    if (pic->lram != 0x8000) {
      if (pic->lram <= 0x0FFF) {
        printf("mem write %#06X: %10s= %#06X\n", pic->lram,
               getFSRname_16E2(pic->lram), pic->ram[pic->lram]);
      } else {
        if (pic->lram >= 0x8000) {
