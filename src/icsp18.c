...
int pic_icsp18(_pic *pic) {
  ...
  if ((icsp_bit == 5) && (!icsp_cmd)) {
    ...
    case 0x00: // Core Instruction                 0000
      if (pic->print)
        printf("_Core Instruction\n");
      break;
    case 0x02: // Shift out TABLAT register        0010
      if (pic->print)
        printf("_Shift out TABLAT register\n");
    ...
    case 0x08: // Table Read                       1000
      ...
      tblptr = ((*pic->P18map.TBLPTRU) << 16) | ((*pic->P18map.TBLPTRH) << 8) |
               (*pic->P18map.TBLPTRL);
      ...
      // read config
        ...
        if (tblptr & 0x0001)
          ...
          icsp_cmddata |= (pic->config[(tblptr - 0x300000) >> 1] & 0x00FF)
                          << 11;
      }
      // read device id
      ...
      // read debug vector
        ...
        if (tblptr & 0x0001)
          ...
          icsp_cmddata |= (pic->debugv[(tblptr - 0x200028) >> 1] & 0x00FF)
                          << 11;
      } else
        (*pic->P18map.TABLAT) = 0;

      if (pic->print)
        printf("READ====0x%04lX ==========0x%04X\n", tblptr,
               (icsp_cmddata & 0x00007F800) >> 11);
    ...
    case 0x09: // Table Read, post-increment       1001
      if (pic->print)
        printf("_Table Read, post-increment\n");
    ...
    case 0x0A: // Table Read, post-decrement       1010
      if (pic->print)
        printf("_Table Read, post-decrement\n");
    ...
    case 0x0B: // Table Read, pre-increment        1011
      if (pic->print)
        printf("_Table Read, pre-increment\n");
    ...
    case 0x0D: // Table Write, post-increment by 2 1101
      if (pic->print)
        printf("_Table Write, post-increment by 2\n");
      break;
    case 0x0E: // Table Write, post-decrement by 2 1110
      if (pic->print)
        printf("_Table Write, post-decrement by 2\n");
      break;
    case 0x0F: // Table Write, start programming   1111
      if (pic->print)
        printf("_Table Write, start programming\n");
      break;
    default:
      if (pic->print)
        printf("_Unknow Command %02X \n", (icsp_cmddata & 0x00000F));
      icsp_cmddata = 0;
      icsp_bit = 0;
      icsp_cmd = 0;
      break;
    ...
    case 0x02: // Shift out TABLAT register        0010
      if (pic->print)
        printf("Shift out TABLAT register\n");
      break;
    case 0x08: // Table Read                       1000
      if (pic->print)
        printf("Table Read\n");
    ...
    case 0x09: // Table Read, post-increment       1001
      if (pic->print)
        printf("Table Read, post-increment\n");
      break;
    case 0x0A: // Table Read, post-decrement       1010
      if (pic->print)
        printf("Table Read, post-decrement\n");
      break;
    case 0x0B: // Table Read, pre-increment        1011
      if (pic->print)
        printf("Table Read, pre-increment\n");
      break;
    case 0x0C: // Table Write                      1100
      if (pic->print)
        printf("Table Write\n");
      ...
      if (pic->print)
        printf("WRITE====0x%04lX ==========0x%04X\n", tblptr,
               (icsp_cmddata & 0x0000FFFF0) >> 4);

      break;
    case 0x0D: // Table Write, post-increment by 2 1101
      if (pic->print)
        printf("Table Write, post-increment by 2\n");
      tblptr = ((*pic->P18map.TBLPTRU) << 16) | ((*pic->P18map.TBLPTRH) << 8) |
               (*pic->P18map.TBLPTRL);
      if ((tblptr >> 1) < pic->ROMSIZE) {
        pic->prog[tblptr >> 1] = ((icsp_cmddata & 0x000FFFF0) >> 4);
    ...
    case 0x0E: // Table Write, post-decrement by 2 1110
      if (pic->print)
        printf("Table Write, post-decrement by 2\n");
      tblptr = ((*pic->P18map.TBLPTRU) << 16) | ((*pic->P18map.TBLPTRH) << 8) |
               (*pic->P18map.TBLPTRL);
      if ((tblptr >> 1) < pic->ROMSIZE) {
        pic->prog[tblptr >> 1] = ((icsp_cmddata & 0x000FFFF0) >> 4);
    ...
    case 0x0F: // Table Write, start programming   1111
      if (pic->print)
        printf("Table Write, start programming\n");
      tblptr = ((*pic->P18map.TBLPTRU) << 16) | ((*pic->P18map.TBLPTRH) << 8) |
               (*pic->P18map.TBLPTRL);
      if ((tblptr >> 1) < pic->ROMSIZE) {
        pic->prog[tblptr >> 1] = ((icsp_cmddata & 0x000FFFF0) >> 4);
