...
int
read_ihx_16e(_pic * pic, const char * fname, int leeprom)
{
 FILE* fin;
 int lc = 0;
 unsigned int bc;
             ...
             if (((addrh << 16) | addr) >= 0x10000)
              {
               ...
               for (bc = 0; bc < nbytes; bc++)
                {
                 addrx = ((addrh << 16) | addr) + bc - 0x10000;
                 if ((addrx / 2) < pic->IDSIZE)
                  mptr[addrx] = parse_hex (line + 9 + (bc * 2), 2);
                }
              }
   ...
   for (i = 0; i < pic->ROMSIZE; i++)
    {
     ...
     if (nb == 0)
      ...
      {
       snprintf (tmp, 200, "%s%02X%02X", values, pic->prog[i]&0x00FF, (pic->prog[i]&0xFF00) >> 8);
       strcpy (values, tmp);
      }
...
int
write_ihx16e(_pic * pic, const char * fname)
{
   ...
   for (i = 0; i < pic->IDSIZE; i++)
    {
     ...
     if (nb == 0)
      ...
      {
       snprintf (tmp, 200, "%s%02X%02X", values, pic->id[i]&0x00FF, (pic->id[i]&0xFF00) >> 8);
       strcpy (values, tmp);
      }
   ...
   for (i = 0; i < pic->CONFIGSIZE; i++)
    {
     ...
     if (nb == 0)
      ...
      {
       snprintf (tmp, 200, "%s%02X%02X", values, pic->config[i]&0x00FF, (pic->config[i]&0xFF00) >> 8);
       strcpy (values, tmp);
      }
   ...
   for (i = 0; i < pic->EEPROMSIZE; i++)
    {
     ...
     if (nb == 0)
      ...
      {
       snprintf (tmp, 200, "%s%02X", values, pic->eeprom[i]);
       strcpy (values, tmp);
      }
