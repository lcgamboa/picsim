...
int serial_cfg(_pic* pic, int nser) {
    ...
    if ((pic->serial[nser].serial_BAUDCTL == NULL) || (!((*pic->serial[nser].serial_BAUDCTL) & 0x08))) {
        ...
        if (*pic->serial[nser].serial_TXSTA & 0x04)  // BRGH=1
        {
            pic->serial[nser].serialexbaud =
                pic->freq / (4 * (((*pic->serial[nser].serial_SPBRGH) << 8) + (*pic->serial[nser].serial_SPBRG) + 1));
        } else {
            pic->serial[nser].serialexbaud =
                pic->freq / (16 * (((*pic->serial[nser].serial_SPBRG) << 8) + (*pic->serial[nser].serial_SPBRG) + 1));
        }
    }
...
unsigned long serial_recbuff(_pic* pic, int nser, unsigned char* c) {
    ...
    if (pic->serial[nser].flowcontrol) {
        ...
        if ((pic_get_pin(pic, pic->serial[nser].ctspin) == 0) && (pic->serial[nser].bc > 0)) {
...
void serial(_pic* pic, int nser) {
    ...
    if ((*pic->serial[nser].serial_RCSTA & 0x80) == 0x80) {
        ...
        if ((pic->serial[nser].serial_BAUDCTL == NULL) || (!((*pic->serial[nser].serial_BAUDCTL) & 0x08))) {
            ...
            if (*pic->serial[nser].serial_TXSTA & 0x04) {
                ...
                if (pic->serial[nser].serialc >=
                    ((((*pic->serial[nser].serial_SPBRGH) << 8) + (*pic->serial[nser].serial_SPBRG) + 1) * 10)) {
                    sr = 1;
                }
            } else {
                // BRGH=0  start + 8 bits + stop
                if (pic->serial[nser].serialc >=
                    ((((*pic->serial[nser].serial_SPBRGH) << 8) + (*pic->serial[nser].serial_SPBRG) + 1) * 40)) {
                    sr = 1;
                }
            }
        }
        ...
        if (sr) {
            ...
            if (((*pic->serial[nser].serial_TRIS_RX & pic->serial[nser].serial_TRIS_RX_MASK) !=
        ...
        switch (pic->family) {
            case P16:
                pic_wr_pin16(pic, pic->usart_tx[nser],
                             bb_uart_io(&pic->serial[nser].bbuart, pic->pins[pic->usart_rx[nser] - 1].value));
                break;
            case P16E:
