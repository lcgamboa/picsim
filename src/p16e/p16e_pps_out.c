...
static void set_pps(_pic *pic, unsigned char val, unsigned char *port,
                    unsigned char pin) {
  int pinn = -1;
  ...
  if (pic->processor == P16F18855) // TODO make better selection
  {
    ...
    case LATxy:
      ...
      if (pic->sck == pinn)
        pic->sck = 0;
      if (pic->usart_tx[0] == pinn)
        pic->usart_tx[0] = 0;
      if (pic->ccp[1].pin == pinn)
        pic->ccp[1].pin = 0;
      if (pic->ccp[0].pin == pinn)
        pic->ccp[0].pin = 0;
      if (pic->ccp[2].pin == pinn)
        pic->ccp[2].pin = 0;
      if (pic->ccp[3].pin == pinn)
        pic->ccp[3].pin = 0;
      if (pic->ccp[4].pin == pinn)
        pic->ccp[4].pin = 0;
      break;
    }
  } else // PIC16F18324
