        ...
        (pic->pins[pic->t1cki - 1].value == 1)))) // TMRICS=t1cki  TMREN=1
  {
    ...
    if (pic->cp1 >= fpw2[((*pic->P16map.T1CON) & 0x30) >> 4]) {
      ...
      if ((*pic->P16map.TMR1L) == 0) {
        if (((*pic->P16map.TMR1H) + 1) == 0x100)
          (*pic->P16map.PIR1) |= 0x01; // TMR1IF
        (*pic->P16map.TMR1H)++;
      }
      pic->cp1 = 0;
    }
    ...
    if (((pic->CCPCOUNT >= 1) && (pic->ccp[0].pin > 0)) &&
        ((*pic->P16map.CCP1CON) & 0x0C) == 0x08) {
      if (((*pic->P16map.TMR1H) == (*pic->P16map.CCPR1H)) &&
          ((*pic->P16map.TMR1L) == (*pic->P16map.CCPR1L))) // match !!
      {
    ...
    if ((pic->CCPCOUNT >= 2) && (pic->ccp[1].pin > 0) &&
        (((*pic->P16map.CCP2CON) & 0x0C) == 0x08)) {
      if (((*pic->P16map.TMR1H) == (*pic->P16map.CCPR2H)) &&
          ((*pic->P16map.TMR1L) == (*pic->P16map.CCPR2L))) // match !!
      {
        ...
        case 3: // trigger special event
          (*pic->P16map.TMR1H) = 0;
          (*pic->P16map.TMR1L) = 0;
          if ((*pic->P16map.ADCON0) & 0x01)
            (*pic->P16map.ADCON0) |= 0x04; // if ad on, enable one conversion
          break;
        }
      }
    }
    ...
    if ((pic->CCPCOUNT >= 3) && (pic->ccp[2].pin > 0) &&
        (((*pic->P16map.CCP3CON) & 0x0C) == 0x08)) {
      if (((*pic->P16map.TMR1H) == (*pic->P16map.CCPR3H)) &&
          ((*pic->P16map.TMR1L) == (*pic->P16map.CCPR3L))) // match !!
      {
