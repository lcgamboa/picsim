  ...
  if ((pin - 1) < pic->PINCOUNT) {
    ...
    if ((pic->pins[(pin - 1)].pord >= 0) && (pic->pins[(pin - 1)].port)) {
      ...
      if ((pic->pins[(pin - 1)].value) == 1)
        ...
        (*pic->pins[(pin - 1)].port) &= ~val;
#ifdef ICSPDBG
      if ((pic->pins[(pin - 1)].port == pic->P18map.PORTB) &&
          ((*pic->P18map.DEBUG) & 0x80)) {
        (*pic->P18map.DEBUG) =
            ((*pic->P18map.DEBUG) & 0xF3) |
...
int pic_dir_pin18(_pic *pic, unsigned char pin, unsigned char dir) {
  ...
  if ((pic->pins[pin - 1].pord >= 0) && (pic->pins[pin - 1].port)) {
    ...
    tris = sfr_addr(pic->pins[pin - 1].port) +
           (sfr_addr(pic->P18map.TRISA) - sfr_addr(pic->P18map.PORTA));
...
void periferic18_step_in(_pic *pic) {
  // int i;
...
void periferic18_step_out(_pic *pic) {
  int i, val;
  ...
  if (pic->lram == sfr_addr(pic->P18map.DEBUG)) {
    ...
    if (((*pic->P18map.DEBUG) & 0x80) == 0x80) {
      (*pic->P18map.TRISB) =
          ((*pic->P18map.TRISB) & 0x3F) | (((*pic->P18map.DEBUG) & 0x03) << 6);

      if (!((*pic->P18map.DEBUG) & 0x02))
        (*pic->P18map.PORTB) = ((*pic->P18map.PORTB) & 0x7F) |
                               (((*pic->P18map.DEBUG) & 0x08) << 4);
      if (!((*pic->P18map.DEBUG) & 0x01))
        (*pic->P18map.PORTB) = ((*pic->P18map.PORTB) & 0xBF) |
                               (((*pic->P18map.DEBUG) & 0x04) << 4);

      // printf("pgd=%i tpgd=%i pgc=%i
  ...
  if (pic->getconf(CFG_DEBUG)) // DEBUG ON
  {
    ...
    if (((*pic->P18map.DEBUG) & 0x80) == 0) {
      ...
      if (((*pic->P18map.DEBUG) & 0x20) == 0x20)
        pic->sstep++;

      if (((pic->ocd_pgca == 1) && (pic->ocd_pgc == 0)) || // HardWare
          (((pic->pc >> 1) ==
            (((((*pic->P18map.BDMSR2) & 0x0F) << 16) |
              ((*pic->P18map.BDMSR1) << 8) | (*pic->P18map.BDMSR0)) /*+1*/)) &&
           (pic->dbg > 3)) || // Break Point
          ((((*pic->P18map.DEBUG) & 0x20) == 0x20) &&
           (pic->sstep == 3))) // step
      {
        ...
        //         \n");                                   //HardWare
        //         if((pic->pc>>1) ==
        //         (((((*pic->P18map.BDMSR2)&0x0F)<<16)|((*pic->P18map.BDMSR1)<<8)|(*pic->P18map.BDMSR0))/*+1*/
        //         )) printf("Break Point\n");      //Break Point
        ...
        if (pic->pc != 0x00000) {
          ...
          if (((*pic->P18map.STKPTR) & 0x1F) > 0) {
            ...
            (*pic->P18map.TOSU) =
                ((pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1]) & 0x1F0000) >>
                16;
            (*pic->P18map.TOSH) =
                ((pic->stack[((*pic->P18map.STKPTR) & 0x1F) - 1]) & 0x00FF00) >>
                8;
          } else {
            (*pic->P18map.TOSL) = 0;
            (*pic->P18map.TOSH) = 0;
         ...
         */
        (*pic->P18map.TRISB) = ((*pic->P18map.TRISB) & 0x3F) |
                               (((*pic->P18map.DEBUG) & 0x03) << 6);
  ...
  if (((*pic->P18map.PORTA) != pic->porta) ||
    ...
    for (i = 0; i < pic->PINCOUNT; i++) {
      if ((pic->pins[i].pord >= 0) && (pic->pins[i].port))
        ...
        case PD_IN:
          val = 0x01 << (pic->pins[i].pord);
          port = sfr_addr(pic->pins[i].port) +
                 (sfr_addr(pic->P18map.LATA) - sfr_addr(pic->P18map.PORTA));
          pic->pins[i].lvalue = ((pic->ram[port] & val) != 0); // latch
          if (pic->pins[i].ptype > 2)
            pic_wr_pin18(pic, i + 1, 0);
          else
            pic_wr_pin18(pic, i + 1, pic->pins[i].value);
          break;
        default:
          break;
        }
    ...
    if (pic->P18map.PORTF)
      pic->portf = (*pic->P18map.PORTF);
    if (pic->P18map.PORTG)
      pic->portg = (*pic->P18map.PORTG);
  }
    ...
    for (i = 0; i < pic->PINCOUNT; i++) {
      if ((pic->pins[i].pord >= 0) && (pic->pins[i].port)) {
        ...
        if ((pic->ram[tris] & val) == 0) {
          ...
          pic_wr_pin18(pic, i + 1, pic->pins[i].lvalue);
        } else {
          val = pic->pins[i].dir;
          pic->pins[i].dir = PD_IN;
          if (val != PD_IN)
            pic_wr_pin18(pic, i + 1, pic->pins[i].ovalue);
        }
      }
    }
    pic->trisa = (*pic->P18map.TRISA);
