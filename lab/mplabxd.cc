     ...
     if(sockfd < 0)
      ...
      reply=0x00; 
	   ...
	   printf("SETBK cmd\n");	
           ...
           break;
         case STRUN:
           if (send (sockfd, (char *)&Window1.picdbg, 1, 0) != 1)
           { 
             printf ("send error : %s \n", strerror (errno));
             ret=1;
             reply=0x01; 
...
#endif
           break;
         case PROGI://TODO implement prog IDS
#ifdef _DEBUG_
	   printf("PROGI cmd\n");	
#endif
           break;
         case READD:
           if (send (sockfd, (char *)pic->ram, pic->RAMSIZE, 0) != pic->RAMSIZE)
           { 
             printf ("send error : %s \n", strerror (errno));
             ret=1;
             reply=0x01; 
	   ...
	   printf("READD cmd\n");	
           ...
           break;
         case READP:
           if (send (sockfd, (char *)pic->prog, 2*pic->ROMSIZE, 0) !=  2*pic->ROMSIZE)
           { 
             printf ("send error : %s \n", strerror (errno));
             ret=1;
             reply=0x01; 
...
#endif
           break;
         case READC:  //TODO implement read config
#ifdef _DEBUG_
	   printf("READC cmd\n");	
#endif
           break;
         case READI: //TODO implement read IDS
#ifdef _DEBUG_
	   printf("READI cmd\n");	
#endif
           break;
